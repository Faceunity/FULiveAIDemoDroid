        ...
        FURenderManager.registerFURender(context, authpack.A(), new OperateCallback() {
            ...
            public void onSuccess(int i, String s) {
                if (i == FURenderConfig.OPERATE_SUCCESS_AUTH) {
                    /**
                     * 设置算法人脸模块的加载策略，是否关闭一些模块的加载。可提高加载速度。在loadAIProcessor之前
                     ...
                     *   FUAIFACE_DISABLE_RACE = 1 << 4,  //关闭人种
                     */
                    switch (FUConfig.DEVICE_LEVEL) {
                        case FuDeviceUtils.DEVICE_LEVEL_MINUS_ONE:
                        case FuDeviceUtils.DEVICE_LEVEL_ONE:
                            FUAIKit.getInstance().fuSetFaceAlgorithmConfig(AICommonData.FUAIFACE_DISABLE_FACE_OCCU | AICommonData.FUAIFACE_DISABLE_SKIN_SEG | AICommonData.FUAIFACE_DISABLE_DEL_SPOT );
                            break;
                        case FuDeviceUtils.DEVICE_LEVEL_TWO:
                            FUAIKit.getInstance().fuSetFaceAlgorithmConfig(AICommonData.FUAIFACE_DISABLE_SKIN_SEG | AICommonData.FUAIFACE_DISABLE_DEL_SPOT);
                            break;
