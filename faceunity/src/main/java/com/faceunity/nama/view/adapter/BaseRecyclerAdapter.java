package com.faceunity.nama.view.adapter;
...
public abstract class BaseRecyclerAdapter<T> extends RecyclerView.Adapter<BaseRecyclerAdapter.BaseViewHolder> {
     ...
     * 单选或不选 可取消
     */
    public static final int SINGLE_CAN_CANCEL_CHOICE_MODE = 2;

    /**
     * 多选
     */
    public static final int MULTI_CHOICE_MODE = 3;
    // 单选默认选中的位置
    ...
    private boolean mCanManualClick = true;
    ...
    public void setCanManualClick(boolean canManualClick) {
        mCanManualClick = canManualClick;
    }
    ...
    public void setItemsSelected(@NonNull List<T> data) {
        if (data.size() > 0) {
            for (T datum : data) {
                int pos = indexOf(datum);
                setItemSelectedMulti(pos);
            }
        }
    }
    ...
    @IntDef({NO_CHOICE_MODE, SINGLE_CHOICE_MODE, SINGLE_CAN_CANCEL_CHOICE_MODE, MULTI_CHOICE_MODE})
    ...
    public class InnerItemViewClickListener extends OnMultiClickListener {
        ...
        protected void onMultiClick(View view) {
            ...
            } else if (choiceMode == SINGLE_CAN_CANCEL_CHOICE_MODE) {
                boolean isClicked = view.isSelected();
                if (isClicked) {//如果当前是选中的
                    mSelectedItems.remove(mLastSelected);
                    notifyItemChanged(mLastSelected);
                    mLastSelected = DEFAULT_SELECTED_POSITION;
                } else {//如果不是选中的那个就更新单选按钮
                    mSelectedItems.put(position, selectedData);
                    if (isValidPosition(mLastSelected) && mLastSelected != position) {
                        mSelectedItems.remove(mLastSelected);
                    }
